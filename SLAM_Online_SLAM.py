...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Omega = matrix()
    Xi = matrix()
    size = 2+2*num_landmarks
    matrix.zero(Omega,size, size)
    matrix.zero(Xi,size,1)

    size += 2

    # Set the inicial position of the robot
    for k in range(2):
        Omega.value[k][k] = 1
        Xi.value[k][0] = world_size / 2.0
    ...
    for i in range(len(data)):
        ...
        # Expand matrices for the new moviment
        list = [0,1] + range(4, 2*(1+num_landmarks)+2)
        Omega = Omega.expand(size,size,list,list)
        Xi = Xi.expand(size,1,list,[0])

        # Go through the motion
        for k in range(2):
            Omega.value[k][2+k] -= 1 / motion_noise
            Omega.value[2+k][k] -= 1 / motion_noise
            Omega.value[k][k] += 1 / motion_noise
            Omega.value[2+k][2+k] += 1 / motion_noise
            Xi.value[k][0] -= motion[k] / motion_noise
            Xi.value[2+k][0] += motion[k] / motion_noise

        # Go through the measurements
        for j in range(len(data[i][0])):
            for k in range(2):
                b = 2*(2 + measurement[j][0])
                Omega.value[k][b+k] -= 1 / measurement_noise
                Omega.value[b+k][k] -= 1 / measurement_noise
                Omega.value[b+k][b+k] += 1 / measurement_noise
                Omega.value[k][k] += 1 / measurement_noise
                Xi.value[k][0] -= measurement[j][1+k] / measurement_noise
                Xi.value[b+k][0] += measurement[j][1+k] / measurement_noise

        vector = range(2, len(Omega.value))
        A = Omega.take([0,1],vector)
        B = Omega.take([0,1])
        C = Xi.take([0,1],[0])

        Omega = Omega.take(vector)-A.transpose()*B.inverse()*A
        Xi = Xi.take(vector,[0])-A.transpose()*B.inverse()*C
