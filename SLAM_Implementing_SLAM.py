...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(len(data)):
        ...
        for k in range(2):
            Omega.value[2*i+k][2*(i+1)+k] -= 1 / motion_noise
            Omega.value[2*(i+1)+k][2*i+k] -= 1 / motion_noise
            Omega.value[2*(i+1)+k][2*(i+1)+k] += 1 / motion_noise
            Omega.value[2*i+k][2*i+k] += 1 / motion_noise
            Xi.value[2*i+k][0] -= motion[k] / motion_noise
            Xi.value[2*(i+1)+k][0] += motion[k] / motion_noise

        # Go through the measurements
        for j in range(len(data[i][0])):
            for k in range(2):
                Omega.value[2*i+k][2*(N + data[i][0][j][0])+k] -= 1 / measurement_noise
                Omega.value[2*(N + data[i][0][j][0])+k][2*i+k] -= 1 / measurement_noise
                Omega.value[2*(N + data[i][0][j][0])+k][2*(N + data[i][0][j][0])+k] += 1 / measurement_noise
                Omega.value[2*i+k][2*i+k] += 1 / measurement_noise
                Xi.value[2*i+k][0] -= measurement[j][1+k] / measurement_noise
                Xi.value[2*(N + data[i][0][j][0])+k][0] += measurement[j][1+k] / measurement_noise
